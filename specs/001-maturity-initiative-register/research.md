...
> **Amended HAP-11 (Q-026):** the per-parent complement rule above, taken alone, did not fully realise this decision's own stated goal — a red-team proof showed a node suppressed at one level is recoverable by summing PUBLISHED nodes at other levels (e.g. all-HIG − a published group = a suppressed sub-4 sibling). HAP-11 added `HierarchySuppression.Close` (run after the per-parent pass inside the shared `RollupPipeline`, so it binds live reads AND the frozen close snapshot): equal-membership collapse + a fixpoint over the tree identities until no suppressed node's count is recoverable by differencing the published set a reader can reach. This **realises D2's rationale ("closes the subtraction attack") hierarchy-globally**; it strengthens, not reverses, this decision (no DR). The **as-built** description of record is `docs/wiki/cycles-and-assessment.md` (DR-0003); this note is a forward pointer so a future reader of D2 does not read the one-level rule as the whole protection.
