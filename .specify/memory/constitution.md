...
3. **The bill:** the session-cost hook appends token spend per session to `.claude/cost-log.csv`, tagged to the story via the branch name. Gitignored, never hand-edited. Joined with worklogs → $/story and $/hour — the platform's own delivery becomes an AI-adoption data point HIG can cite.
