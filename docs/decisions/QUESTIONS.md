...
**Provisional answer in effect (per CLAUDE.md §6.3):** retention erasure nulls the three nullable value fields and **zeroes `SelfScore`** (`AssessmentScore.Erase()`); the moderated aggregate for the cycle is already frozen in `RollupSnapshot` at close (closed cycles read snapshots, never recompute from `AssessmentScore`), so zeroing raw values after 3 years never affects any published figure. **Idempotency + "was this erased" is tracked by the per-assessment `RetentionErasure` audit row (the authoritative ledger), NOT by row content** — so the ambiguity of a zeroed-vs-genuine-0 score does not affect correctness: a second retention run skips any assessment that already has a `RetentionErasure` row and affects zero rows.

**Blocks:** nothing locally (synthetic data). **G1 owner-ratification requested:** confirm zeroing `SelfScore` (with the audit-row erasure ledger) is acceptable, or require a nullable/tombstone score column — an L2 schema/migration change (a new story; would also let the export mark erased dimensions explicitly). If ruled to need a distinct tombstone, the fallback is a `nullable SelfScore` or an `Erased` flag column plus a one-line migration and a test update; the retention job's cycle-selection, audit-ledger idempotency, and fail-closed atomicity are unaffected.

**HAP-12 panel round-1 additions (2026-07-22) — fold into the owner's column/tombstone decision:**
- **Export now discloses erasure (B1, shipped):** the right-of-access export cross-references the `RetentionErasure` ledger and presents an erased datum as `null` + `Erased: true` (`ExportScore`) / `DataErased: true` (`ExportCycle`), never a fabricated `0`. A domain guard (`AssessmentScore.Erase()` sets an in-memory, EF-ignored `Erased` flag; `SetManager`/`AdoptSelf` throw `AssessmentScoreErasedException`) refuses moderating an erased row within a unit of work. **Caveat:** the flag is transient per load, so it does not survive a reload — the authoritative cross-request "was erased" signal remains the audit ledger; persisting the marker (so a late-override re-moderation across requests is refused at the domain) is exactly the nullable/tombstone-or-`Erased`-column decision requested here.
- **Idempotency nuance (updated — now fail-CLOSED):** the ledger parser was subsequently changed to THROW `CorruptErasureLedgerException` on a malformed/empty `RetentionErasure` `Detail` (was: silently skipped). So an unparseable ledger row can never cause a display read to serve an erased score as genuine, nor a silent re-erase — the read fails closed. A persisted erased marker would additionally make idempotency independent of Detail parsing.
- **Scale nuance (real-data onboarding precondition, not local):** `RetentionService` loads ALL `RetentionErasure` details into memory each run (unbounded over years), and `SeamAssessmentStore.RunRetentionErasureAsync` loads all candidate assessments in one transaction. Both are trivial at synthetic scale but must be **batched** (paged erasure + a ledger index/marker column) before the real-data unlock — record this as a real-data-onboarding precondition alongside the column decision.
## 2026-07-22 · HAP-11 (owner acceptance testing) · Q-028 — Individual-contributor dashboard scope + own-team labelling

Found in owner acceptance testing: a plain individual contributor (no direct reports) who opens **Dashboard** is resolved by `GET /api/me/dashboard` (HAP-11 `ResolveDefaultNodeAsync`, "else → own team") to **their own team node — which is their manager's team, named after the manager** (e.g. George, an IC, sees a dashboard titled "Chloe Yilmaz"). This is **by-design and privacy-safe** — the tile is an aggregate that returns `suppressed:"N<4", figures:null` until 4+ people are moderated, so no individual data (and nothing about the manager) leaks; it is HAP-11 AC's explicit `/me/team/summary` "own team, suppressed if <4". But acceptance testing surfaced two rough edges the owner should rule on:

1. **Labelling:** a *member's* own-team dashboard is titled with the **manager's name**, which reads as "you are viewing your manager's dashboard" rather than "your team (led by X)". A neutral label ("Your team") would remove the confusion.
2. **Scope:** should a plain IC have a **Dashboard** nav entry / team-rollup view at all? The shell shows "Dashboard" to every role; for an IC it only ever renders a perpetually-suppressed team tile. Reasonable both ways (team-transparency vs. "ICs only self-assess"). Ties to FR-022 scope and the k=4 differencing floor (a member of a team of exactly 4 can partially difference their own team's published aggregate — a documented G1 residual, made concrete by this path).
...
## 2026-07-23 · HAP-18 (dev) · Q-031 — FR-061 reminder cadence needs a cycle's planned close date, which Cycle doesn't store

FR-061's non-responder-reminder / escalation cadence ("7/3/1 days **before close**; escalations from 3 days before close") requires knowing an Open cycle's FUTURE close date. But HAP-7's `Cycle` never captures a planned close: `CreateCycleRequest` takes no date, and `ClosesAt` is stamped only retroactively when `/close` is actually called (forward-only state machine). So "days until close" is uncomputable for a genuinely Open cycle. Root spec / FR-002 / FR-061 / data-model.md all speak of cycles having "open/close dates" as if both are known together, but HAP-7 only ever records the close date at the moment of close. HAP-18 is the first story to actually need the planned date.

**Provisional answer in effect (per CLAUDE.md §6.3):** HAP-18 implements the cadence against a **configured intended-close reference held as configuration, not a DB column** (consistent with the cadence values T-7/T-3/T-1 themselves already being config, and the Q-005 weights-as-config precedent) — derived from the cycle's monthly identity or a configured cycle-length from `OpenedAt`. No schema change, so HAP-18 stays migration-free and HAP-15 remains the sole in-flight migration.
**Owner action:** ratify whether the notification cadence keying off a *configured* intended-close is acceptable, or whether `Cycle` should gain an explicit nullable `ScheduledCloseAt` (set at Create/Open) — the cleaner long-term model, but a future migration story (it would also let FR-002/FR-061 stop treating the planned close as implicit). Calibration risk (notification timing), not a correctness/privacy issue.
**Status:** OPEN (provisional in effect)

## 2026-07-23 · HAP-15 (dev diagnosis) · Q-032 — HAP-11 HierarchySuppression.Close tie-break is non-deterministic (sacred-code robustness)

Found by dev-hap15b while diagnosing a flaky HAP-15 integration test. HAP-11's `HierarchySuppression.Close` (the cross-level differencing defence, BR1) must, when a suppressed node's value is recoverable by subtraction across levels, sacrifice (suppress) one node to break the differencing path. When **multiple candidate nodes tie** (e.g. AllHig / Portfolio / Group-chain / a BU / a Team all at N=4 — which arises when exactly one BU has a scored population and its N equals every upper total), the choice of which node to suppress falls through to internal node-array ordering that tracks `Dictionary`/`HashSet` enumeration over freshly-generated GUIDs — so it is **non-deterministic run-to-run**. Verified by hand-trace + an isolated unit test against `HierarchySuppression.Close` + an instrumented integration run (BU_A observed N=4 Suppressed=Complement while all upper nodes stayed published at N=4, the tie firing as analysed).

**Is it a leak?** No, not in production as shipped: suppression is computed **once at cycle close and frozen** into `RollupSnapshot` (HAP-10), so a given closed cycle always serves the SAME published/suppressed set — no cross-run/query-time differencing is possible. The leak invariant (no individual score recoverable) holds on every single computation; only the *choice* of which node absorbs the suppression varies between independent computations. So **G1's witnessed guarantee stands** (it was witnessed on frozen, deterministic-per-close output).

**Why it still matters:** (a) it makes any suppression-dependent test non-reproducible/flaky (the presenting symptom); (b) it's a robustness gap on sacred seam code — a *stable* tie-break (e.g. order candidates by a deterministic key like node id) would guarantee reproducible suppression; (c) defence-in-depth: if suppression were ever recomputed live for the same cycle (it must not be, for closed cycles), differing choices across computations could theoretically enable cross-computation differencing.

**Provisional answer in effect:** no code change now — pre-existing, out of scope for the L2 HAP-15, and not an active leak. HAP-15 worked around it by making its own test data deterministic (giving both BUs a real scored population, removing the artificial tie).
**Owner/next-L3 action:** spin a small **L3** follow-up to make `HierarchySuppression.Close`'s tie-break deterministic (stable candidate ordering), with **hap-red-team + hap-domain-specialist** confirming the deterministic choice preserves the no-leak invariant in the tie cases. Flag to red-team on the next story that touches suppression (HAP-19 reads completion, not suppression, but its L3 red-team should be made aware).
**Status:** OPEN (provisional in effect; L3 follow-up recommended)

## 2026-07-23 · HAP-15 (L2 domain panel) · Q-033 — how to collapse a BU's measured floor distribution to a single "measured level" for declared-vs-measured divergence

The HAP-15 domain review caught a blocking bug: `MeasuredFloorLevel` computed `floor(min(per-dimension means))` — "floor of the mean," which violates the floor rule (averaging across people before flooring lets cross-person strength mask a per-person weakness; root spec Appendix A: "a rounded-mean label is WRONG — the floor supersedes it"). Fixed to derive from the already-published, per-person `FloorLevelDistribution` (keeps it L2 — same `RollupReads` figures, no new score query).

But that leaves a real semantics choice: the declaration is a single BU level (0-3), so the "measured" side of the FR-065/FR-047 divergence needs a single representative level collapsed from the floor **distribution**. Options: modal per-person floor (the typical member), minimum (most conservative — any one weak member drags the BU down, mirroring the floor rule's "weakest wins" most literally), a percentile/threshold ("level N only if ≥X% of people clear it"), or reporting the distribution itself rather than a scalar. Each yields a different divergence figure that HAP-16 will report to group leadership.
...
## 2026-07-23 · HAP-18 (QA finding + fix) · Q-035 — synth seeds no BuDelegate grants, so BU-lead notifications (and BU-level individual features) are dormant on synthetic data

QA (qa-hap18) found an L3 privacy leak: HAP-18's BU-lead summary (FR-061, assessment participation) and BU-lead escalation (FR-037, register) picked their recipient from HierarchyRoleResolver's **depth-from-root "BU Lead" label** with no structural entitlement check — so on a non-uniform tree (Q-014's known limitation) a mislabeled person receives a BU's participation data they're denied a direct `AuthorizeIndividualRead` of. Fixed by re-anchoring the recipient on the seam's genuine BU anchor (an explicit BU-scoped `OrgRole.BuDelegate` grant, DB-re-read — exactly what `AssessmentReads.ClassifyReader` uses), fail-closed (skip + count when no grant).

**Consequence surfaced:** the synthetic build **deliberately seeds no `BuDelegate` grants** (DR-0005 / Q-014 — BU leads are depth-identified only, and the seam already fail-closes a grant-less BU lead to ordinary-Manager reach). So after this fix, on pure synthetic data **every** BU-lead summary/escalation now SKIPS (SkippedNoLead) — there is no seam-entitled BU lead to send to. This is the fail-closed-CORRECT outcome (sending to the depth-label lead was the leak), and it mirrors the seam's existing behaviour (a grant-less "BU lead" already gets no BU-level individual reach — cf. the G1 witness where BU Lead Thomas was denied George). But it means the FR-047/FR-061 "BU Lead receives per-team summary" behaviour is only demonstrable once a `BuDelegate` grant exists (the tests create one); on a pure-synth G1/G2 demo, no BU-lead summary fires.

**Provisional answer in effect (per CLAUDE.md §6.3):** accept the fail-closed dormancy on synthetic data — it is privacy-correct and consistent with DR-0005's "BU-level features activate at real-data onboarding" precondition. No depth-label fallback (that was the leak).
**Owner action:** decide whether the **synthetic generator should seed `BuDelegate` grants for BU leads** (so BU-lead notifications + BU-level individual features demonstrate on synth, and G1/G2 can witness them) — this would be an L2 synth-generator change + likely a DR — OR whether BU-lead-level features remain intentionally dormant until real-data onboarding. Ties to Q-014 (depth-label ratification) and DR-0005.
**Status:** OPEN (provisional in effect)

## 2026-07-23 · HAP-18 (QA final, non-blocking) · Q-036 — the visibility seam's `ClassifyReader.FirstOrDefault` under-grants a multi-BU delegate

HAP-18's BU-lead-summary recipient gate (`CycleReminderJob.BuLeadPassesSeamReadGate`) binds each candidate to the seam's `AssessmentReads.AuthorizeIndividualRead` over the candidate's full grant set, sampling **one** non-responder in the BU and generalising (correct for the realistic single-BuDelegate case: BU-lead reach is BU-wide, so a pass on one member ⇒ pass on all). QA (qa-hap18-final) noted the sampling is NOT exact if a recipient co-holds a **second** `BuDelegate` grant over a *different* BU: `ClassifyReader` anchors that reader via `FirstOrDefault`, so the seam itself may permit them to read only their first-anchored BU — while the person genuinely holds a BuDelegate grant over the counted BU too. The gate can then pass (via the sampled subject / an `isDirectManager` disjunct) and deliver the counted BU's summary.

**Why this is NOT a HAP-18 leak (and why closure proceeded):** the recipient is `buLeadMap[buId]`, which `RoleGrantBuLeadResolver` populates ONLY from a `BuDelegate(buId)` grant — so the summary never reaches anyone lacking a genuine BuDelegate entitlement to the counted BU. The divergence is against the **seam's own** `FirstOrDefault` behaviour, which *under*-grants a multi-BU delegate (a reader holding `BuDelegate(A)` + `BuDelegate(B)` is arguably entitled to both, but the seam anchors them to one). It is non-deterministic (grant order) and dormant on synth (Q-035, no BuDelegate grants seeded).

**Provisional answer in effect (per CLAUDE.md §6.3):** ship as-is — no unentitled disclosure occurs. Optional exactness follow-up for the notification gate: check every counted person, or pass the specific anchor BU to `AuthorizeIndividualRead`.
**Owner action (L3 seam question, not just notifications):** decide whether `ClassifyReader` should consider ALL of a reader's `BuDelegate` grants (multi-BU reach) rather than `FirstOrDefault` — this is a seam-semantics decision affecting every BU-scoped read path, ties to Q-014 and DR-0005, and would be an L3 change with its own panel. Until then the seam's single-anchor behaviour stands and the notification gate is safe.
**Status:** OPEN (provisional in effect)
