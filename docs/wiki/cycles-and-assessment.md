...
### Rollup read endpoints & dashboard (HAP-11 — FR-013/015-019/041)

Aggregate reads live in the seam (`RollupReads` over `RollupEndpoints`): `GET /api/bus/{id}/dashboard` (BU Lead), `GET /api/org/{type}/{id}/rollup` (Group/Portfolio/Exec), `GET /api/me/team/summary` (own team), and `GET /api/me/dashboard` — which resolves the caller's richest scope (Exec→AllHig, Portfolio/Group Leader→their node, BU Lead→their BU, else→own team) and re-dispatches through the same scope-checked reads. **Open cycle** figures are computed **live** via `RollupPipeline`; **closed** cycles read the **frozen** `RollupSnapshot` — both call the identical `ComputeNodes`, so they agree by construction. Scope is enforced by `AggregateScope`/`RollupReads.ScopeAsync` off the FR-022 hierarchy anchors (Q-024, G1-ratify); out-of-scope / nonexistent nodes → **404** (existence-leak convention). Every figure is suppression-gated: a suppressed node returns `figures:null` + reason, never a number.

- **UI** (`app/src/screens/dashboard-bu`, components `StatTile`/`DimensionBar`/`TrendSparkline`/`SuppressedCell`): floor + mean StatTiles with trend arrows, seven DimensionBars (values printed at bar end), a `TrendSparkline` (aria-hidden with values in adjacent text), completion tile, and stubbed initiative slots ("—", FR-041, until HAP-13). The floor KPI is derived from `floorLevelDistribution` (the honest "% at L1+ / % at L0" — never `round(mean)`). `SuppressedCell` shows the reason, never zero/blank or the literal count (F2 overrides the mockup's "n=3<4"). Tokens-only, colour never the sole signal, vitest-axe WCAG 2.2 AA.
- **Open rulings:** Q-025 (FR-018 asks per-dimension histograms "to surface bimodal teams"; the delivered node-floor-distribution + per-dimension means is mockup-faithful but may not fully satisfy that — owner ruling, provisional-in-effect). G1 residuals for owner ratification: cross-cycle trend differencing, the inherent k=4 floor, and the laminar-partition assumption (a real matrix/dual-manager directory would need an import-time guard).
