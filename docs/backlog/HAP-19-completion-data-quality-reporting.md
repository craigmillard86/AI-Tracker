...
## Acceptance criteria
- [ ] Completion per team/BU/cycle: invited, submitted, moderated, unmoderated %, contractor-excluded counts — asserted against hand-computed synth values; trend across closed cycles (FR-019).
...
- [ ] Data-quality score per BU (FR-038): defined as documented weights of update timeliness (% active initiatives current) + field completeness (% required value/governance fields populated); computation unit-tested; surfaced on the BU dashboard slot and via API.
